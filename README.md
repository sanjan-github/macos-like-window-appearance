...
The experimental (`extra`) and custom-radius rounding options are removed. Those modes used `SetWindowRgn` and caused visual artifacts. Rounding is limited to Windows 11's documented preferences: `native`, `small`, and `default`. Saved `extra` or `custom` values fall back to `native`.
